...
// ===== CUENTAS (pestana "Cuentas" del mismo Sheet) =====
let cuentasCsv = '';
try { cuentasCsv = String($('Consultar Cuentas').first().json.data || ''); } catch(e){}
const cuentas = [];
if (cuentasCsv) {
  const filasC = parseCSV(cuentasCsv);
  // Encabezado esperado: Pais, Metodo, Disponible, Datos, Notas
  for (let i=1; i<filasC.length; i++) {
    const row = filasC[i];
    const pais = (row[0]||'').trim();
    const datos = (row[3]||'').trim();
    if (!pais || !datos) continue;
    const disp = /^s/i.test((row[2]||'').trim()); // "Si"/"si" -> true
    cuentas.push({ Pais: pais, Metodo: (row[1]||'').trim(), Disponible: disp, Datos: datos });
  }
}
const cuentasDisponibles = cuentas.filter(c => c.Disponible);

return [{ json: { lista, modo, esMayorista, appUrl:APP_URL, hayEfectivoVe, totalPaises: paises.length, cuentas: cuentasDisponibles, hayCuentas: cuentasDisponibles.length>0, error: paises.length? null : 'Lista vacia' } }];
'@
...
$prepCode = @'
// Prepara los datos para el debounce con Redis: numero, texto, token unico y nombres de clave.
let jid='', texto='';
...
$nodes = @(
  ...
  [ordered]@{
    ...
    type = 'n8n-nodes-base.code'; typeVersion = 2; position = @(-20,128); id = 'c1a2b3c4-1111-4a11-9111-prep00000001'; name = 'Prep Debounce'
  },
  [ordered]@{
...
$connections = [ordered]@{
  ...
  'Es mensaje de cliente?' = @{ main = @( ,@( @{ node='Prep Debounce'; type='main'; index=0 } ) ) }
...
$json | Out-File -FilePath "C:\Users\USER\Documents\APP PARA CAMBIOS JK CONVERSOR\workflow_bot_whatsapp_v7.json" -Encoding utf8
Write-Output ("JSON v7 generado. Longitud: " + $json.Length + " chars. Nodos: " + $nodes.Count)
