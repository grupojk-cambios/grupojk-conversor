...
// Efectivo Venezuela: el CSV trae muchas sub-entradas "Efectivo Venezuela-Estado" (todas en USD, misma tasa base).
// Las colapsamos en UNA sola opcion en USD para que la IA la pueda ofrecer; guardamos los estados disponibles aparte.
const evEntries = paises.filter(p=>/^efectivo venezuela/i.test(p.nombre));
let evEstados = [];
if (evEntries.length) {
  const rep = evEntries[0];
  const setE = {};
  evEntries.forEach(p => { const partes = String(p.nombre).split('-'); const est = (partes[1]||'').trim(); if (est) setE[est] = true; });
  evEstados = Object.keys(setE);
  lista.push({ Pais:'Efectivo Venezuela', Codigo:'USD', Moneda:'Dolar efectivo en Venezuela', Envio: tEnv(rep)? Number(tEnv(rep).toFixed(2)):'N/D', Recibo: tRec(rep)? Number(tRec(rep).toFixed(2)):'N/D', EsDolar:true });
}
...
return [{ json: { lista, modo, esMayorista, appUrl:APP_URL, hayEfectivoVe, evEstados, totalPaises: paises.length, cuentas, hayCuentas: cuentas.length>0, diaIndex, error: paises.length? null : 'Lista vacia' } }];
'@
...
- VENEZUELA tiene DOS servicios distintos, NO los confundas: (1) "Venezuela" = bolivares (VES) a cuenta/pago movil. (2) "Efectivo Venezuela" = DOLARES en EFECTIVO entregados en mano. Si el cliente habla de dolares, "$", "verdes", efectivo, cash, o menciona una ciudad/estado venezolano (Caracas, Maracaibo, Valencia, Maracay, etc.), usa SIEMPRE "Efectivo Venezuela" (USD), NUNCA "Venezuela" (bolivares). Si dice "bolivares", "Bs", "pago movil" o "a cuenta", usa "Venezuela". Si no esta claro, pregunta: "Lo quieres en bolivares o en efectivo (dolares)?". Para efectivo, pregunta en que estado/ciudad lo retira.
...
- COMPROBANTE DE PAGO: si ves "[El cliente envio un COMPROBANTE de pago...]", NUNCA digas "no tengo esa cuenta". Responde: "Recibi tu comprobante! 🙌 ¿A que cuenta(s) enviamos tu remesa (banco, numero y titular)?". Si en la conversacion el cliente YA te dio la cuenta destino, confirma: "Listo! Tu pago y la cuenta destino estan registrados, procesamos tu remesa enseguida ✅". La cuenta destino es donde el destinatario RECIBE el dinero; el cliente puede dividir el monto en varias cuentas.
...
{{ $node["Code in JavaScript"].json.hayEfectivoVe ? '- Efectivo Venezuela disponible en: ' + ($node["Code in JavaScript"].json.evEstados || []).join(', ') + '. Pregunta el estado/ciudad antes de cerrar.' : '' }}
...
const combinado = String($('Combinar').item.json.mensajeCombinado || '').toLowerCase();
...
try { jid = $('Webhook Evolution API').first().json.body.data.key.remoteJid || ''; nombre = $('Webhook Evolution API').first().json.body.data.pushName || 'Cliente'; numero = String(jid).replace(/\D/g,''); } catch(e){}
return [{ json: { aprobado: true, jid, nombre, numero, mensajeCombinado: combinado } }];
'@
...
$nodes = @(
  ...
  [ordered]@{
    parameters = @{ method = 'POST'; url = 'http://evolution-api:8080/message/sendMedia/bot-JK-prueba-operador1'; authentication = 'genericCredentialType'; genericAuthType = 'httpHeaderAuth'; sendHeaders = $true; headerParameters = @{ parameters = @( @{ name = 'Content-Type'; value = 'application/json' } ) }; sendBody = $true; specifyBody = 'json'; jsonBody = '={ "number": "593961230380-1601657528@g.us", "mediatype": "image", "mimetype": "{{ $json.mimetype || "image/jpeg" }}", "media": "{{ $json.base64 }}", "caption": "Cliente: {{ $("Detectar Aprobación").item.json.nombre }}\nNumero: +{{ $("Detectar Aprobación").item.json.numero }}\n\n{{ $("Detectar Aprobación").item.json.mensajeCombinado }}" }'; options = @{} }
...
$connections = [ordered]@{
  ...
  'Detectar Aprobación'    = @{ main = @( ,@( @{ node='Redis Leer ID Imagen'; type='main'; index=0 } ) ) }
  ...
  'Enviar Imagen Grupo'    = @{ main = @( ,@( @{ node='Enviar Sticker Cliente'; type='main'; index=0 } ) ) }
}
